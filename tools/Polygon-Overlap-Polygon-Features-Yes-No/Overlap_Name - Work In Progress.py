#A tool that inputs the name of one polygon feature (from input) overlaps 
#another polygon feature (fcName) with the name of the input feature
#Written by Jesse Fraser (jesse.fraser@gov.bc.ca)
#Started May 5th 2020

#Still need to figure out a way to deal with input polygons that have overlapping features
...
#The layer to append the Name to
fcName = arcpy.GetParameterAsText(0)

#The layer that will be iterated through to provide Name
...
with arcpy.da.UpdateCursor(lyr_Overlap, [field_name]) as cursor:
	for test in cursor:
		...
		if num > 0:
			...
			#Populate via the field name
			pop_value = r'"' + test[0] + r'"'
			
			arcpy.CalculateField_management(lyr_union, new_field, pop_value, "PYTHON_9.3")
