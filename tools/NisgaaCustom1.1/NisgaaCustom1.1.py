...
import os, arcpy, shutil, openpyxl, time, datetime, xlrd, xlwt
...
def GetOriginalGDB():
    ...
    OriginalGDB = ''
    while arcpy.Exists(OriginalGDB) is False:

        try:
            OriginalGDB = input('Please enter the file path to the original GDB, in " ".')
...
def CopyXlsxTemplate():
    ...
    MyReport=os.path.join(Container, 'MyReport%s' % Today + '.xlsx')
    if not os.path.exists(MyReport):
...
def GetXlsxTemplate():
    ...
    TemplateReport=''
    while os.path.exists(TemplateReport) is False:

        try:
            TemplateReport = input('Please enter the file path to the template spreadsheet, in " ".')
...
def GetTempFolder():
    ...
    MyTempContainer=os.path.join(Container+ '\\xxxTemp%s' % Today)
...
def GetStudyArea():
    ...
    print ' getstudyarea--------------------------------------------------------------------------------------------------------------'
    #print 'Enter Study Area:'
    #StudyArea = input()
    StudyArea = 'Nisgaa_Nass_Wildlife_Area'
##    try:
##        StudyArea = input('Please enter the study area layer name, in " ".')
##
##    except:
##        NameError
##        print 'That was not entered within "quotation" marks.'
...
def CheckStudyArea():
    ...
    if arcpy.Exists(StudyAreaPath):
        global StudyAreaPath

    else:
        print 'error with study area'
